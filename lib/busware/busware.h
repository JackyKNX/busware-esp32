...
public:
    GenericTransceiver(HardwareSerial* serial) {
        _serial = serial;
    }
    ...
    int available(void) {
        return (_serial) ? _serial->available() : -1;
    }
    ...
    size_t readBytes(uint8_t *buffer, size_t length) {
        return (_serial) ? _serial->readBytes(buffer, length) : 0;
    }
...
public:
    TPUARTTransceiver(HardwareSerial* serial)
        : GenericTransceiver(serial)
    {
    }
...
public:
    CSMTransceiver(HardwareSerial* serial, uint8_t resetPIN = 0)
        : GenericTransceiver(serial)
    {
        _resetPIN = resetPIN;
    }
...
public:
    TCMTransceiver(HardwareSerial* serial,
                   uint8_t resetPIN = 0,
                   uint8_t turboPIN = 0)
        : GenericTransceiver(serial)
    {
...
public:
    ZigbeeTransceiver(HardwareSerial* serial,
                      uint8_t resetPIN = 0,
                      uint8_t bootloadPIN = 0)
        : GenericTransceiver(serial)
    {
        _resetPIN = resetPIN;
        _bootloadPIN = bootloadPIN;
    }
